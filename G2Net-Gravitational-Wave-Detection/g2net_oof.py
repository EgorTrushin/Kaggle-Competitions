...
OUTPUT_DIR = "/home/egortrushin/Kaggle/g2net/b1_0.8786_0.87439/Checkpoints/"
...
class G2NetClassifier(pl.LightningModule):
    ...
    def forward(self, x):
        z0 = self.spec_layer(x[:, 0, :])
        z1 = self.spec_layer(x[:, 1, :])
        z2 = self.spec_layer(x[:, 2, :])
        x = torch.stack((z0, z1, z2), 1)
        if config["freq_sup"]:
...
for fold in [0,1,2,3,4]:
    ...
    val_df['preds'] = sigmoid(np.array(predictions))
    #all_predictions.append(predictions)
    val_df.to_csv(f'/home/egortrushin/Kaggle/g2net/b1_0.8786_0.87439/f{fold}.csv', index = False)
    del model
    gc.collect()
...
#submission.to_csv('/home/woody/bctc/bctc33/pytorch/b3_new/submission.csv', index = False)
