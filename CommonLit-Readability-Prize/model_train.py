...
EPOCHS = 3
SEED_VAL = 8#1325
VAL_STEP = [100, 100, 100]
DEVICE = torch.device("cuda")
WEIGHT_DECAY = 0.01
WEIGHT_DECAY_CLF = 0.01
NUM_FOLDS = 5
FOLDS_RANDOM_STATE = 2021
GRADIENT_CLIPPING = True
TRAIN_CSV = "~/datasets/commonlitreadabilityprize/train.csv"

model_cfg = {
    "model": "pretrained/clrp_roberta_base",
    "weights_dir": "",
...
for fold in range(NUM_FOLDS):
    ...
    optimizer_parameters = [
            {'params': [p for n, p in model.named_parameters() if "classifier" not in n], 'lr': LR, 'weight_decay_rate': WEIGHT_DECAY},
            {'params': [p for n, p in model.named_parameters() if "classifier" in n], 'lr': LR_CLF, 'weight_decay_rate': WEIGHT_DECAY_CLF},
        ]
