...
def loss_fn(output, target):
    """RMSE loss function"""
    return torch.sqrt(torch.nn.MSELoss()(output.view(-1), target.view(-1)))
...
def train_fold(
    ...
    for epoch_i in range(0, epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (
                step % val_step[epoch_i] == val_step[epoch_i] - 1 or step == len(train_dataloader) - 1
            ) and not step == 0:
                model.eval()
...
def get_oof(df, res):
    """Calculate OOF predictions for given model."""
    oof = np.zeros(len(df))
    for fold in res:
        oof[res[fold]["val_index"]] += res[fold]["preds"]
    return oof
